...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // binding_list
      // binding
      char dummy4[sizeof (std::unique_ptr<VarBinding>)];
    };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_input = 30,                            // input
        S_statement_list = 31,                   // statement_list
        S_statement = 32,                        // statement
        S_statement_2 = 33,                      // statement_2
        S_print_stmt = 34,                       // print_stmt
        S_if_stmt = 35,                          // if_stmt
        S_expr = 36,                             // expr
        S_let_expr = 37,                         // let_expr
        S_binding_list = 38,                     // binding_list
        S_binding = 39,                          // binding
        S_let_body = 40                          // let_body
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_binding: // binding
        value.move< std::unique_ptr<VarBinding> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::unique_ptr<ASTNode>&& v)
        : Base (t)
        , value (std::move (v))
      {}
#else
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::unique_ptr<VarBinding>&& v)
        : Base (t)
        , value (std::move (v))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::unique_ptr<VarBinding>& v)
        : Base (t)
        , value (v)
      {}
#endif
...
switch (yykind)
    {
      ...
      case symbol_kind::S_binding_list: // binding_list
      case symbol_kind::S_binding: // binding
        value.template destroy< std::unique_ptr<VarBinding> > ();
        break;
